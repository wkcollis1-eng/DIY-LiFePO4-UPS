...
## Engineering Basis Document v12
...
**Date:** June 2026
**Revision:** v12 — **Tracks firmware through V1.14.** Builds on the v10 pre-sign-off
fixes (FIX 1, FIX 2) and the v11/V1.13 config simplification, and adds the items
surfaced by the V1.13 pre-deployment review pass. Per the code-review standard, all
...
- **FINDING 1 (Medium) — runtime INA hang was undetectable; "INA-unhealthy in HA"
  ...
  register read the `ina260` platform's `update()` status-warns and returns
  **without publishing** (it never publishes NaN), and `throttle_average` emits
  nothing on an empty window with no NaN received. A mid-operation INA hang
  therefore **freezes** all three `.state` values at the last good reading; the
  isnan()-based "INA260 Healthy" stays TRUE indefinitely, and — because AC keeps
  the API connection alive — the 30-min `api reboot_timeout` never fires either.
  Consequence chain in the *next* outage: `on_battery` never fires at a frozen
  13.3 V → no warnings, **no graceful HA Green shutdown** (unclean filesystem at
  the BP-65 trip); pack protection remains bounded by the existing
  trip → API-loss → 30-min-reboot → watchdog-ride-out chain (§6.7), so this is a
  *detection* gap, not a pack-safety gap — but §6.7's "noticed as INA-unhealthy in
  HA" intervention-window premise held only for boot-time failures (where
  `mark_failed` leaves the sensors never-published → NaN → Healthy correctly OFF).
  **V1.14 adds the stale-data watchdog** (§9.6): `last_ina_publish_ms` updated from
  the bus-voltage `on_raw_value` (fires only on an actual platform publish) and an
  **"INA260 Data Fresh"** binary sensor (OFF after 15 s = three missed 5 s throttle
  windows; millis() unsigned subtraction is rollover-safe). Pair with an HA alert.
- **FINDING 2 (Low) — `-160` watchdog read could race the first AVG=4 conversion
  on a normal boot.** The platform's reset (0x8000) zeroes the bus-voltage register
  and the first averaged result lands only ~5 ms after its config write; `-150`
  inserts its 20 ms settle **only on survival wakes**, so on a fast normal boot the
  `-160` raw read could land inside that window, read 0x0000 → implausible → no
  prevent, leaving the disarm hanging on WiFi alone (timing-dependent rather than
  designed; worst case a healthy device cycle-sleeps through an AP outage on AC —
  self-recovering, no pack risk). **V1.14 retries once after 20 ms** on an
  implausible first read; the cost is paid only on the already-degraded path.
- **FINDING 3 (fixed pre-sign-off) — four stale comments asserted the removed
  priority-600 0x6127 restore** (the same doc/firmware-divergence class that
  produced the V1.13 AVG=1/AVG=4 error). All four rewritten to the V1.13 mechanism
  (platform setup reset + 0x0307 is the sole configurator). Comments-only.
- **NVS flush mechanism reattributed (§8):** the hard commit guarantee on every
  controlled sleep entry is `begin_sleep()` → `App.run_safe_shutdown_hooks()` —
  the restoring-globals component and the preferences `IntervalSyncer` both
  implement `on_shutdown` (globals save → NVS sync; verified in 2026.5.3 source).
  The 100 ms delays let the globals loop detect changed values first; they are
  belt-and-suspenders, not the guarantee. (Uncontrolled brown-out remains the only
  uncovered write-loss path — already accepted under R-6.)
- **Data collection (V1.14, §9.2/§9.6):** survival-wake voltage capture
  (`survival_first/min/last_wake_v`, NVS) — the **field instrument for A-4 / §3.4**:
  the first real survival event records the post-trip resting-OCV profile the
  May 6 telemetry blackout could not see, converting the 13.0 V exit margin from a
  reasoned hedge into measured data; persisted **survival exit reason**
  (recovery / fail-safe / max-cycles — previously log-only, and the logs are
  unreachable mid-outage); `last_outage_duration_min` on a survival recovery now
  includes the slept cycles (was duration-to-arm only); `survival_cycle_count`
  exposed as "Last Survival Cycles". All writes ride the existing per-cycle NVS
  flush — zero added I2C, zero added flushes, ~zero energy.
- **Operational guard:** the two outage-data reset buttons refuse (with a WARN log)
  while `on_battery` is active.
- The wake interval is now single-sourced as `survival_sleep_interval_s` (= 120),
  referenced by both the `deep_sleep` interval and the duration-accounting lambda.
...
**Status:** Implemented — firmware **V1.14** (`ups-monitor-v1_14.yaml`) on the
...
items, the V1.13 config simplification, and the V1.14 review/data items (above)
are the only changes. **No V1.14 item alters a validated entry/exit threshold or
decision branch**; the additions are capture writes inside `-150`, the `-160`
retry, the stale-data watchdog, sensors, and button guards.
**Validation to date:** V1.14 is **config-valid + codegen clean + full ESP-IDF
compile/link CLEAN on ESPHome 2026.5.3** (review environment; sole delta from the
shipped file was `build_path` — details at §13). **Remaining sign-off gates:** the
formal on-host `esphome compile` + flash of V1.14 on the Windows build host (now
de-risked — same ESPHome version linked clean), the §13 V1.14 bench checks, and
the §13 bench measurement of the single unmeasured input that gates R-1
(below-LVD reserve / per-wake burst / sleep floor), which is unchanged and still
open.
...
- **Honest scope — does NOT give multi-day survival on a dead sensor.** In a true
  ...
  window for the human to notice — not 10 days. **v12 correction on what "notice"
  means:** the original "device offline / INA-unhealthy in HA" premise held only for
  *boot-time* INA failure (setup `mark_failed` → sensors never publish → NaN →
  "INA260 Healthy" OFF). A *runtime* hang freezes the values instead (Finding 1,
  header) and was undetectable until V1.14's **"INA260 Data Fresh"** sensor; on AC,
  detection is now Data Fresh going OFF within 15 s (alert in HA); in a post-trip
  outage the device is simply offline, which HA flags as unavailable as before.
...
|100 ms NVS flush on entry *and* re-sleep          |Lets the restoring-globals loop detect the changed arm snapshot / `survival_cycle_count` / V1.14 wake-capture values and queue the saves. **v12 reattribution:** the *hard* commit guarantee is `deep_sleep.enter` itself — `begin_sleep()` runs `App.run_safe_shutdown_hooks()`, and both the restoring-globals component and the preferences `IntervalSyncer` implement `on_shutdown` (globals save → NVS sync; verified in 2026.5.3 source). This covers the FIX-1 watchdog auto-sleep path too. The delay is kept as validated belt-and-suspenders. Uncontrolled brown-out remains the only uncovered write-loss path (accepted, R-6).|
...
|`survival_sleep_interval_s`|`120`  |**V1.14** — single source for the wake interval (seconds); referenced by `survival_sleep_interval` and by the `-150` total-duration lambda (cycles × interval). Change it here only                                                                                  |
|`survival_sleep_interval`  |`${survival_sleep_interval_s}s`|**Timer-wake, primary** (reverted from the 600 s V1.10 backstop). Also the `deep_sleep` component `sleep_duration` (FIX 1, §9.3) so the watchdog auto-sleep wakes on the same timer                                                                  |
...
- **Priority -150** — survival wake decision: **20 ms settle** (V1.13 — now the sole
  ...
  `outage_was_survival`, logs the survival OUTAGE END). **V1.14 additions, all on
  the already-validated branches:** (a) *wake-voltage capture* — on each plausible
  sub-13.0 V wake, write `survival_first_wake_v` (cycle 0), running
  `survival_min_wake_v`, and `survival_last_wake_v` (the charging ≥13.0 V recovery
  sample is deliberately excluded — it is not a rest reading); (b) *exit reason* —
  `survival_exit_reason` set to 1/2/3 at the recovery / fail-safe / max-cycle exits;
  (c) *total duration* — at the recovery exit, `last_outage_duration_min +=
  survival_cycle_count × (120 s ÷ 60)`, converting the field from duration-to-arm to
  grid-fail → recovery-detection (awake windows of ~1–3 s are noise at minute
  resolution). The re-sleep path **powers the INA260 down** (0x00 = 0x6120, ~2 µA)
  and includes a 100 ms NVS flush before `deep_sleep.enter` (120 s) — the same
  flush now also commits the capture fields (zero added writes). No per-exit BOL
  writes (nothing is armed).
...
- **Priority -160 (watchdog confirm)** — if `!survival_mode_active`, do a fresh
  ...
  control), call `deep_sleep.prevent` to disarm the `run_duration` auto-sleep.
  **V1.14:** an implausible *first* read is retried once after 20 ms — on a fast
  normal boot the first read can race the platform's first AVG=4 conversion (~5 ms
  after its setup() config write; the 0x8000 reset zeroes the bus-voltage register
  until then), and `-150` inserts no settle on a normal boot, so without the retry
  the disarm depended on incidental setup timing (Finding 2, header). If still not
  plausible, do not prevent — WiFi `on_connect` may still confirm; otherwise
...
On press: reset `survival_cycle_count` **and the V1.14 capture set**
(`survival_first/min/last_wake_v` = 0, `survival_exit_reason` = 0), write the final
snapshot, set `survival_mode_active` + `outage_was_survival`; then 100 ms flush,
**power the INA260 down** (0x00 = 0x6120, ~2 µA), `deep_sleep.enter` (120 s
timer-wake). The explicit `deep_sleep.enter` is `manual=true`, so it sleeps
regardless of any watchdog prevent.

### 9.5 Globals (survival)

`survival_mode_active` (NVS), `survival_should_sleep` (transient),
`survival_cycle_count` (NVS), `outage_was_survival` (NVS); **V1.14:**
`survival_first_wake_v` / `survival_min_wake_v` / `survival_last_wake_v` (NVS),
`survival_exit_reason` (NVS), `last_ina_publish_ms` (transient, §9.6).

### 9.6 INA260 Stale-Data Watchdog (V1.14)

**Why (Finding 1, header):** the `ina260` platform never publishes on a failed
read and `throttle_average` emits nothing on an empty no-NaN window
(both source-verified, 2026.5.3), so a runtime hang freezes `.state` and defeats
any isnan()-based health check. **Mechanism:** `last_ina_publish_ms` is stamped
from the bus-voltage `on_raw_value` (fires per actual 1 Hz platform publish,
before filters); the **"INA260 Data Fresh"** template binary sensor reads OFF when
no publish has occurred for 15 s (three missed 5 s throttle windows; unsigned
millis() subtraction is rollover-safe at the 49.7-day wrap; reads OFF before the
first publish of a boot, which is correct — there is no data yet). **Division of
labour:** "INA260 Healthy" (isnan) covers boot-time failure; "Data Fresh" covers
runtime failure; the `-150`/`-160` raw plausibility reads remain the loop-side
...
|**Runtime INA hang freezes telemetry undetected; "INA260 Healthy" stays TRUE; graceful HA shutdown silently lost in the next outage (V1.13)**|**Medium (was latent)**|**Detection FIXED (V1.14, Finding 1)** — `ina260` platform never publishes on a failed read; `throttle_average` emits nothing on an empty window (both source-verified) → isnan() health check cannot see a runtime hang. V1.14 "INA260 Data Fresh" trips ≤15 s after the last real publish. **Residual:** the sensor restores the intervention window only if an HA alert is configured and acted on; pack safety was already bounded by the trip → API-loss → 30-min reboot → watchdog ride-out chain (§6.7)|
|**`-160` watchdog first read races the platform's first AVG=4 conversion on a fast normal boot → disarm depends on incidental setup timing (V1.13)**|**Low (was latent)**|**FIXED (V1.14, Finding 2)** — one retry after 20 ms on an implausible first read; cost paid only on the already-degraded path. Worst pre-fix case was a healthy device cycle-sleeping through an AP outage on AC (self-recovering, no pack risk). §9.2|
|**Four stale comments asserted the removed priority-600 0x6127 restore (V1.13)**             |**Low (correctness)**      |**FIXED (V1.14, Finding 3)** — all four rewritten to the platform-as-sole-configurator mechanism. Comments-only; same divergence class that produced the AVG=1/AVG=4 error V1.13 corrected                                                                          |
|**Outage-data reset buttons pressed during an active outage wipe the live record**           |**Low (data only)**        |**FIXED (V1.14)** — both buttons refuse with a WARN log while `on_battery` is active                                                                                                                                                                                |
...
|Post-trip resting OCV (A-4)     |Gates the 13.0 V exit margin (exit safety; §5.2)                                          |Out-of-band DMM rest reading (§13) **or, as of V1.14, the first real (or bench) survival event** — the wake-voltage capture (§9.6 trio) records first/min/last rest readings at 120 s cadence, converting A-4 to measured data with no extra instrumentation|
...
|**`esphome compile` clean — host link (V1.14)**|**V1.14 is config-valid + codegen clean + full ESP-IDF compile/link CLEAN on ESPHome 2026.5.3** (review environment, June 10 2026: `esphome config` → codegen → compile, link succeeded in 473 s; all V1.14 items verified present in generated `main.cpp`, including the intact FIX-1 `set_sleep_duration(120000)`, all three exit-reason writes, the wake-capture and duration lambdas, the `-160` retry, the `add_on_raw_state_callback` watchdog wiring, and both button guards). **Sole delta from the shipped file:** `build_path` (Linux `/tmp/...` vs the Windows path). Remaining formality: re-run on the Windows build host and flash — de-risked from "never linked" to "linked clean on the identical ESPHome version, differing only in build_path".|**Gating for V1.14 sign-off (de-risked; host re-run + flash)**|
...
|**Stale-data watchdog trips on a runtime hang (V1.14)**|On AC, after a healthy boot, hold the INA260 in reset (or lift SDA): confirm Battery V/I/P freeze at their last values, "INA260 Healthy" stays ON (demonstrating the Finding-1 mechanism), and **"INA260 Data Fresh" goes OFF within ~15 s** with the device staying awake (WiFi confirmation holds the watchdog disarmed). Restore the INA; confirm Data Fresh returns ON.|High|
|**Wake-voltage capture populates and survives recovery (V1.14)**|Run a bench survival cycle (≥2 sleep cycles), restore AC: confirm `survival_first/min/last_wake_v` hold plausible sub-13.0 V rest values after the recovery boot, first ≤ last (relaxation rises), min ≤ both, and the ≥13.0 V recovery wake did NOT overwrite `survival_last_wake_v`.|High|
|**Exit reason records correctly (V1.14)**|After the bench recovery above, "Survival Exit Reason" reads *Recovery*; separately, disconnect the INA mid-sleep and confirm it reads *Fail-safe* after the fail-safe boot.|Medium|
|**Total duration includes slept cycles (V1.14)**|After the bench recovery, confirm `last_outage_duration_min` ≈ duration-to-arm + (cycles × 2 min), matching "Last Survival Cycles".|Medium|
|**-160 retry does not regress normal boots (V1.14)**|Cold boot on AC with a healthy INA: confirm the priority -160 "INA usable … watchdog disarmed" log fires on the FIRST read (no retry log), i.e. the retry path is dormant in normal operation.|Medium|
...
exit-safety measurement. **As of V1.14, the firmware itself is also an instrument
for this quantity:** the §9.6 wake-voltage capture records first/min/last rest
readings at 120 s cadence through any survival episode (bench or field), so the
first such event closes A-4 with measured data even if the DMM reading is never
taken. The one µA-meter reading remains the highest-value
...
**Net effect (V1.14):** survival is the validated ~15-day timer-wake design at the
...
- ESPHome `i2c` / `deep_sleep` / `globals` component documentation; and the
  ...
  behaviour.
- **v12 source verifications (ESPHome 2026.5.3):** `ina260.cpp` — `setup()` =
  reset 0x8000 → delay(2) → ID check (0x5449/0x2270) → config 0x0307; `update()`
  on a failed read status-warns and returns **without publishing** (basis for
  Finding 1). `sensor/filter.cpp` — `ThrottleAverageFilter` interval handler
  outputs NAN only when `n_ == 0 && have_nan_`; an empty no-NaN window emits
  nothing (second leg of Finding 1). `deep_sleep_component.cpp` —
  `begin_sleep()` → `App.run_safe_shutdown_hooks()` → component teardown →
  powerdown hooks → `deep_sleep_()`; with the restoring-globals component and
  preferences `IntervalSyncer` both implementing `on_shutdown` (basis for the §8
  flush reattribution).
