...
## Engineering Basis Document v10
...
**Date:** June 2026
**Revision:** v10 — **Applies two pre-sign-off fixes surfaced by the V1.11
engineering review, producing firmware V1.12.** The survival entry/exit logic is
byte-for-byte the V1.9.2/V1.11 design (validated against the May 6 event); only the
two defects below changed. Both were caught *during review, before sign-off*, per
...
**Status:** Implemented — firmware **V1.12** (`ups-monitor-v1_12.yaml`) on the
UPS-Monitor-THT Rev 1 PCB. The survival path is the validated V1.9.2 logic; the
`run_duration` watchdog (V1.11) plus the two V1.12 fixes are the only changes.
**Validation to date:** config valid + full ESPHome **codegen clean on 2026.5.3**
(the review verified the brick fix in the generated source and confirmed no C++/codegen
error from either fix). **Remaining sign-off gates:** the final on-host `esphome
...
(V1.12 survival logic is byte-for-byte V1.9.2/V1.8 — see §6, §7). Event-driven
...
**Latest survival estimate (V1.12) is the §6.2 timer-wake figure: ~15 days at the
...
The mechanisms actually present (V1.12):
...
## 9. Implementation — Firmware V1.12
...
- **Priority 600** — restore INA260 continuous mode (0x6127) + 10 ms freshness
  delay; **log the wake cause** (timer vs cold — validation aid). **No BOL alert
  writes** (event-wake removed; the 0x06/0x07 map is retained only as a reference
  comment).
...
- **`on_battery` `on_release` (FIX 2, V1.12)** — the normal-outage finalizer; its
  `last_outage_ah/wh/duration` recompute is gated on `outage_start_uptime > 0` so a
  stray survival-recovery `on_release` (see §8) cannot overwrite the arm-time
  snapshot. Recharge-peak capture is unconditional.
...
- `ina260_alert` binary_sensor is retained as a **single-owner** passive input
  ...
  as a possible INA/I2C fault — nothing in V1.12 should ever assert it.
...
|`esphome compile` not yet run on host                                                       |Low–Medium                 |**Reduced** — config valid + full codegen clean on ESPHome 2026.5.3, both fixes confirmed in generated `main.cpp`, no C++/codegen error. **Open:** the final on-host link + flash (the toolchain link step was not run in review) — §13                              |
...
|**`esphome compile` clean — host link (V1.12)**|Config + full **codegen are confirmed clean on ESPHome 2026.5.3** (both fixes present in generated `main.cpp`). Remaining: run the final `esphome compile` (C++ link) on the build host and flash. (Review could not run the link — the toolchain download was unavailable in that environment.)                                                       |**Gating for V1.12 sign-off**                     |
...
divider + a routed ADC pin), out of scope for V1.12 firmware, and is the recommended
...
**Net effect (V1.12):** survival is the validated ~15-day timer-wake design at the
