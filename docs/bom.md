# Bill of Materials

All prices as-purchased (March 2026). Amazon prices subject to change; Pololu prices are typically stable.

---

## Main Power Components

| Item | Vendor | Unit Price | Notes |
|---|---|---|---|
| Mean Well HDR-60-12 PSU | Amazon | $26.53 | Incl. $2.40 shipping. 4.5A/54W, 88% peak eff. (~87% at 22% operating load), OVP 13.8–16.2V |
| Cyclenbatt 12V 10Ah LiFePO4 Battery | Amazon | $29.99 | Built-in 10A BMS, 5000+ cycles, F2 spade terminals |
| Victron BatteryProtect BP-65 12/24V | Amazon | $36.53 | LVD: Setting 7 = 11.8V cutoff / 12.8V reconnect. MOSFET, IP67, 5-yr warranty |
| Ideal Diode 4-60V 10A, 2 pcs | Pololu | $7.58 | Pololu #5382. Only 1 required. Prevents PSU backfeed from battery |

## Monitoring & Control

| Item | Vendor | Unit Price | Notes |
|---|---|---|---|
| Shelly Plus Uni | Amazon | $18.99 | 2× ADC, DS18B20 support, WiFi, HA native integration |
| DS18B20 1M Temperature Sensor, 2 pcs | Amazon | $4.48 | Waterproof 1m cable, 1-Wire protocol |

## Protection & Wiring

| Item | Vendor | Unit Price | Notes |
|---|---|---|---|
| Mini Inline Fuse Holder 16AWG, 6 pcs | Amazon | $4.99 | For F1–F4 |
| Mini (Dual Blade) APM/ATM Fuse Assortment | Amazon | $4.66 | 2×10A (F1+F2), 1×5A (F4), 1×2A (F3) |
| 16AWG Wire 3' Black / 3' Red | Amazon | $5.55 | All wiring is 16AWG |
| 2 Pack DC Barrel Jack 5.5mm×2.1mm 18AWG 36" | Amazon | $7.51 | HA Green load connections |
| Comcast/Xfinity DC Barrel Jack | Xfinity | — | Free from Xfinity Store |
| 3' 16/3 Power Cord w/Pigtail | Amazon | $5.99 | AC input to PSU |
| 16-14AWG Nylon Female Spade Connectors | Amazon | $4.49 | Battery F2 terminal connections |
| Screw Terminal Block 2 Pin, 5mm Pitch, 4 pcs | Pololu | $1.89 | Ideal diode connections |
| 2 In 4 Out Terminal Block Module | Amazon | $9.99 | Battery bus |

## Enclosure & Hardware

| Item | Vendor | Unit Price | Notes |
|---|---|---|---|
| LeMotech Junction Box 10"×7.9"×4.7" | Amazon | $21.84 | IP65 ABS (exterior dims; interior 9.6"×7.6"×4.5"), includes mounting hardware (5% discount applied) |
| 1/2" NPT Cable Glands, 5-Pack | Amazon | $4.99 | AC and DC cable entries |
| Velcro Heavy Duty, 2"×4", 3 sets | Amazon | $4.93 | PSU and Victron BP-65 wall mounting |

## Tools

| Item | Vendor | Unit Price | Notes |
|---|---|---|---|
| Wire Strippers / Crimping Tool | Amazon | $4.74 | 16-14AWG |
| Soldering Kit 60W, 110V | Amazon | $7.99 | For terminal connections |

---

## Vendor Summary

| Vendor | Items | Amount |
|---|---|---|
| Amazon | All components except Pololu | $204.19 (sub-total) |
| Pololu | Ideal diode (×2), screw terminal blocks, USPS shipping ($6.45) | $15.92 |
| Amazon Cash Back (5%) | — | −$10.21 |
| Sales Tax | — | $13.98 |
| **TOTAL** | — | **$223.88** |

## Cost by Category

| Category | Cost | % of Total |
|---|---|---|
| Main Power Components (PSU, Battery, BP-65, Ideal Diode) | $100.63 | 45.0% |
| Monitoring & Control (Shelly, DS18B20) | $23.47 | 10.5% |
| Protection & Wiring | $44.94 | 19.7% |
| Enclosure & Hardware | $32.83 | 14.6% |
| Tools | $12.73 | 5.7% |
| Pololu Shipping | $6.45 | 2.9% |
| Amazon Cash Back (5%) | −$10.21 | — |
| Sales Tax | $13.98 | 6.2% |
| **TOTAL** | **$223.88** | 100% |

> Tools ($12.73) are one-time costs not specific to this project and can be excluded if already on hand, reducing the effective build cost to ~$211.
